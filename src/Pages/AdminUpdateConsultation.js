...
const UpdateAppointment = () => {
  ...
  const handleUpdate = async () => {
    try {
      // Set confirm to true before sending the update request
      const updatedBooking = {
        ...booking,
        confirmed: true, // Setting confirm to true
      };

      await axios.put(`http://localhost:8080/api/consultations/${id}`, updatedBooking);
      alert("Appointment updated successfully!");
      navigate(-1); // Redirect to homepage or listing page
    } catch (error) {
      console.error("Failed to update booking:", error);
      alert("Failed to update appointment. Please try again.");
    }
          ...
          <table className="min-w-full text-sm text-white">
            <thead className="text-white border-b border-gray-600">
              <tr>
                <th className="px-6 py-4 text-center">Client Name</th>
                <th className="px-6 py-4 text-center">Email</th>
                <th className="px-6 py-4 text-center">Phone</th> {/* Added Phone Column */}
                <th className="px-6 py-4 text-center">Preferred Date</th>
                <th className="px-6 py-4 text-center">Preferred Time</th>
                <th className="px-6 py-4 text-center">Consultant Name</th>
                <th className="px-6 py-4 text-center">Action</th>
  ...
  {booking.phoneIsd ? `+${booking.phoneIsd} ${booking.phoneNo}` : booking.phoneNo}
</td>
