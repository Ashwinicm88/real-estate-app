...
const MultiStageForm = () => {
  ...
  const nextStage = async () => {
    ...
    try {
      ...
      if (!Array.isArray(formData.oneBHKConfigs)) {
        setFormData((prev) => ({ ...prev, oneBHKConfigs: [] }));
      }
      ...
      if (stage === 0) {
        ...
        setCachedData((prev) => ({
          ...prev,
          oneBHKConfig: formData.oneBHKConfigs,
        }));
      } else if (stage === 4) {
        setCachedData((prev) => ({
          ...prev,
...
const handleSubmit = async () => {
  try {
    ...
    // Upload Images & Videos first and get URLs
    // const imageFiles = formData.project?.Images || [];
    // const videoFile = formData.project?.Videos?.[0] || null;
    ...
    const payload = {
      ...
      oneBHKConfig: formData.oneBHKConfigs?.map(config => ({
      ...
      twoBHKConfig: formData.twoBHKConfig?.map(config => ({
        typeNumber: config.typeNumber,
        ...
        // bathroom3Area: config.type2bathroom2 || ""
    ...
    // Convert JSON payload to string and append it to FormData
    ...
    stateData.oneBHKConfigs?.forEach((config) => {
      ...
      if (config.type1images?.length) {
        ...
        // ✅ Append OneBHKConfig images & floor plans
    // stateData.oneBHKConfigs?.forEach((config, index) => {
    //   // Append images
    //   const type1Images = config.type1images || [];
    //   type1Images.forEach((file) => {
    //     formDataToSend.append(`oneBHKType1Images`, file);
    //   });

      // Append floor plan images
    //   const type1FloorPlans = config.type1floorplan || [];
    //   type1FloorPlans.forEach((file) => {
    //     formDataToSend.append(`oneBHKType1FloorPlanImages`, file);
    //   });
    // });
...
const uploadFiles = async (files, type) => {
  ...
  return (
    <div>
      ...
      <div className="w-full max-w-4xl bg-black p-4 rounded text-white mx-4 sm:mx-8 md:mx-10 xl:mx-24">
        <LinearProgress
          variant="determinate"
          value={stage === 0 ? 0 : ((stage + 1) / initialSteps.length) * 100}
          className="w-[750px] max-w-[calc(100%-2rem)] mb-2 h-2 mx-10"
          sx={{
                  ...
                  {formData.oneBHKConfigs.map((config, index) => (
                    <div key={index} className="mb-6 border-b pb-4">
                      ...
                      <div className="w-full max-w-4xl grid sm:grid-cols-1 md:grid-cols-2 justify-between mx-auto">
                        <InputField
                          label="Type Number"
                          section={`oneBHKConfigs[${index}]`}
                          field="typeNumber"
                          value={config.typeNumber}
                          onChange={handleChange}
                          error={warnings.oneBHKConfigs?.[index]?.typeNumber}
                          type="number"
                        />
                        ...
                        <InputField
                          ...
                          onChange={handleChange}
                          error={warnings.oneBHKConfigs?.[index]?.type1Units}
                        />
                        ...
                        <InputField
                          ...
                          onChange={handleChange}
                          error={warnings.oneBHKConfigs?.[index]?.type1area}
                        />
                        ...
                        <InputField
                          ...
                          onChange={handleChange}
                          error={warnings.oneBHKConfigs?.[index]?.type1BedroomArea}
                        />
                        ...
                        <InputField
                          ...
                          onChange={handleChange}
                          error={warnings.oneBHKConfigs?.[index]?.type1HallArea}
                        />
                        ...
                        <InputField
                          ...
                          onChange={handleChange}
                          error={warnings.oneBHKConfigs?.[index]?.type1KitchenArea}
                        />
                        ...
                        <InputField
                          ...
                          onChange={handleChange}
                          error={warnings.oneBHKConfigs?.[index]?.type1bathrooms}
                        />
                        ...
                        <InputField
                          ...
                          onChange={handleChange}
                          error={warnings.oneBHKConfigs?.[index]?.type1bathroom1}
                        />
                        ...
                        <InputField
                          ...
                          onChange={handleChange}
                          error={warnings.oneBHKConfigs?.[index]?.type1bathroom2}
                        />
                        ...
                        <InputField
                          ...
                          onChange={handleChange}
                          error={warnings.oneBHKConfigs?.[index]?.type1balcony}
                        />
                        ...
                        <InputField
                          ...
                          onChange={handleChange}
                          error={warnings.oneBHKConfigs?.[index]?.type1parking}
                        />
                        <div>
                          <ImageUpload
                            handleChange={handleChange}
                        ...
                        if (e.target.checked) {
                          setFormData((prev) => ({
                            ...
                            oneBHKConfigs: [
                              ...prev.oneBHKConfigs,
                              {
                                typeNumber: prev.oneBHKConfigs.length + 1,
                                type1Units: "",
                                type1area: "",
                                type1floorplan: [],
                                type1images: [],
