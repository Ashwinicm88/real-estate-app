...
const TimePicker = ({ label, section = "", field, value, onChange, error }) => {
  const timeSlots = [
    { label: "9 AM - 12 PM", value: "9am-12pm" },
    { label: "12 PM - 3 PM", value: "12pm-3pm" },
    { label: "3 PM - 6 PM", value: "3pm-6pm" },
  ...
  return (
    ...
    text-base mobile-s:text-sm mobile-m:text-md mobile-l:text-lg md:ml-10 md:w-3/4 mobile-s:w-3/4">
      ...
      <select
        ...
        value={value}
        onChange={handleTimeChange}
        className={`bg-black text-white w-full p-2 pr-12 focus:outline-none focus:border-white border-b ${error ? "border-red-500" : "border-[#FFFFF]"}`}
      >
        <option value="" disabled>Select a time slot</option>
        {timeSlots.map((slot) => (
          <option key={slot.value} value={slot.value}>
